    ...
    if len(all_acc) > 1:
        ...
        for f in df.keys():
            ...
            results.append({
                ...
                'test_weightf1': round(df_['test_weight_f1'].mean(), 3)
            })
        print(f"Running time: {round((end-start)/len(seeds), 2)}s")

    # Write results to CSV
    with open('../../results/TAPE/Summary.csv', 'a', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=['dataset', 'llm_name', 'num_layers', 'hidden_dim', 'dropout', 'test_acc', 'test_f1', 'test_weightf1'])
        # writer.writeheader()

...
def get_summary (dataset, model_name, index):

    data_type = "articles" if dataset in ["cora", "citeseer"] else "users"
    ...
    neighbor_info = "\none of the central node's neighbors' feature:".join(neighbor_info_list)
    ...
    if model_name == "chatglm3-6b":
        ...
        summary = response["output"]["choices"][0]["message"]["content"].replace('\n', '').strip()
...
def get_response(dataset, model_name, index, write_file_path):
    discription = f"The discription of the node: {graph_data.raw_texts[index]}"
    summary = get_summary(dataset, model_name, index)
...
if __name__ == '__main__':
    ...
    # Create csv file
    llm_neighbor_predfolder = "../results/LLMPredictor/llm_llm_neighbors"
    file_path = f"{llm_neighbor_predfolder}/{args.model_name}/{args.dataset}.csv"
    os.makedirs(llm_neighbor_predfolder, exist_ok=True)
    os.makedirs(f"{llm_neighbor_predfolder}/{args.model_name}", exist_ok=True)

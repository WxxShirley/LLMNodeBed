...
ARXIV_Q = """Question: Which of the following 
        ...
        Reply only one category that you think this paper might belong to. 
        Only reply the category name I given without any other words, please don't use your own words.
        Answer:
    """
...
def cora_response(model_name, index, write_file):
    device = 'cpu'
    data = torch.load(f"../datasets/cora.pt").to(device)

    discription = data.raw_texts[index]
    question = DEFAULT_QUESTION["cora_q"]
...
def pubmed_response(model_name, index, write_file):
    device = 'cpu'
    data = torch.load(f"../datasets/pubmed.pt").to(device)
    discription = data.raw_texts[index]
    question = DEFAULT_QUESTION["pubmed_q"]
...
def citeseer_response(model_name, index, write_file):
    device = 'cpu'
    data = torch.load(f"../datasets/citeseer.pt").to(device)
    discription = data.raw_texts[index]
    question = DEFAULT_QUESTION["citeseer_q"]
...
def wikics_response(model_name, index, write_file):
    device = 'cpu'
    data = torch.load(f"../datasets/wikics.pt").to(device)
    discription = data.raw_texts[index]
    question = DEFAULT_QUESTION["wikics_q"]
...
def arxiv_response(model_name, index, write_file):
    device = 'cpu'
    data = torch.load(f"../datasets/arxiv.pt").to(device)
    discription = data.raw_texts[index]
    question = DEFAULT_QUESTION["arxiv_q"]
...
def instagram_response(model_name, index, write_file):
    device = 'cpu'
    data = torch.load(f"../datasets/instagram.pt").to(device)
    discription = data.raw_texts[index]
    question = DEFAULT_QUESTION["instagram_q"]
...
def reddit_response(model_name, index, write_file):
    device = 'cpu'
    data = torch.load(f"../datasets/reddit.pt").to(device)
    discription = data.raw_texts[index]
    question = DEFAULT_QUESTION["reddit_q"]
...
def acc_f1(dataset, write_file):
    true_labels=[]
    predict_labels=[]
    with open(f"../results/LLMPredictor/llm_zero_shot/prediction/{dataset}.csv", 'r') as file:
...
if __name__ == '__main__':
    ...
    write_file_path = f"../results/LLMPredictor/llm_zero_shot/prediction/{args.dataset}.csv"
    ...
    for index in test_indexes:
        ...
        with open (f"../results/LLMPredictor/llm_zero_shot/prediction/{args.dataset}.csv",'r') as file:
            reader = csv.reader(file)
            for row in reader:
        ...
        if args.dataset == "cora":
            cora_response(args.model_name, index, write_file_path)
        elif args.dataset == "pubmed":
            pubmed_response(args.model_name, index, write_file_path)
        elif args.dataset == "citeseer":
            citeseer_response(args.model_name, index, write_file_path)
        elif args.dataset == "wikics":
            wikics_response(args.model_name, index, write_file_path)
        elif args.dataset == "arxiv":
            arxiv_response(args.model_name, index, write_file_path)
        elif args.dataset == "instagram":
            instagram_response(args.model_name, index, write_file_path)
        elif args.dataset == "reddit":
            reddit_response(args.model_name, index, write_file_path)
        else:
            print("Error, no such dataset.")
            exit(1)

    # calculate acc and f1
    acc_f1(args.dataset,write_file_path)

...
class TextLoraModel(nn.Module):
    ...
    def forward(self, data):
        ...
        label_embeds = self.text_forward(data.label_name)
        ...
        labels = data.y.long().to(self.device) if data.y.dim() == 1 else data.y.squeeze(1).long().to(self.device)
    ...
    def zero_shot_eval(self, node_embeds, label_embeds, data):
        ...
        score_dict = {}
        for cur_R in range(0, self.args.test_R):
            comp_node_embeds = node_embeds[:-1, :]
            comp_node_embeds /= comp_node_embeds.norm(dim=-1, keepdim=True)

            cur_dists = torch.einsum('bn,cn->bc', comp_node_embeds, label_embeds)

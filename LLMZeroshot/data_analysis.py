...
def acc_two_kinds_f1(file_path, dataset):
    ...
    with open(file_path, mode='a', newline='') as file:
        ...
        writer.writerow(
            [f'Accuracy: {accuracy:.3f}, Weighted F1 Score: {weighted_f1:.3f}, Macro F1 Score:{macro_f1:.3f}'])
...
def bias_of_category(dataset, prediction_type, model_name, file_path):
    ...
    if dataset == "cora":
        labels = ["Rule_Learning", "Neural_Networks", "Case_Based", "Genetic_Algorithms", "Theory",
                  "Reinforcement_Learning", "Probabilistic_Methods", "Others"]
    elif dataset == "citeseer":
        labels = ["Agents", "ML (Machine Learning)", "IR (Information Retrieval)", "DB (Databases)",
                  "HCI (Human-Computer Interaction)", "AI (Artificial Intelligence)", "Others"]
    else:

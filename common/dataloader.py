...
def re_split_data(num_node, train_percent=0.6, val_percent=0.2, test_percent=0.2, device="cuda:0"):
    ...
    test_ids = np.sort(node_ids[int(num_node * (train_percent + val_percent)): int(
        num_node * (train_percent + val_percent + test_percent))])
...
def load_graph_dataset(dataset_name, device, re_split=False, path_prefix="../.."):
    ...
    if dataset_name in ["computer", "history", "photo"]:
        if re_split:
            graph_data.train_mask, graph_data.val_mask, graph_data.test_mask = re_split_data(graph_data.num_nodes,
                                                                                             device=device)
        else:
...
def load_graph_dataset_for_gnn(dataset_name, device, re_split=False, path_prefix="../..", emb_model="shallow"):
    ...
    if emb_model != "shallow":
        ...
        node_feat = torch.load(f"{path_prefix}/datasets/{emb_model}/{dataset_name}.pt", map_location=device).to(
...
def load_graph_dataset_for_tape(dataset_name, device, re_split=False, use_gpt=False, gpt_name="Qwen-3B"):
    graph_data = load_graph_dataset(dataset_name, device, re_split, path_prefix="../..")
...
def load_graph_dataset_for_zerog(dataset_name, device, re_split=False, path_prefix="../.."):
    graph_data = torch.load(f"{path_prefix}/datasets/{dataset_name}.pt").to(device)
    # Alternative
    # graph_data.edge_index = to_undirected(graph_data.edge_index) if dataset_name in ["citeseer", "arxiv"] else graph_data.edge_index
    # graph_data.edge_index = to_undirected(graph_data.edge_index)
